...
const Nav: React.FC = () => {
    ...
    const [postModal, setPostModal] = useState({
        addPost: { open: false, fadeOut: false }
    })
    ...
    return (
        <>
            <div className="lg:p-16 lg:w-96 lg:h-screen lg:border-r-2 overflow-auto">
                <div className={`p-4 bg-neutral-50 fixed inset-0 transition-transform duration-500 transform ${secondaryNavOpen ? 'translate-y-0' : 'translate-y-full'} lg:static lg:translate-y-0 lg:p-0 z-10`}>
                    <nav className="flex flex-col gap-2 mx-auto max-w-sm h-full justify-center">
                        ...
                        <div>
                            {user && (
                                <NavLink to={ `/user/${user.id}` } className={({ isActive }) => isActive ? "secondary-nav-item-container nav-active" : "secondary-nav-item-container"}>
                        ...
                        <div>
                            <button className="button secondary-nav-item-container w-full" onClick={ () => handlePopups({ modalKey: "addPost", setModals: setPostModal }) }>
                                <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" aria-hidden="true">
                        ...
                        <div className="mt-4">
                            <button className="button secondary-nav-item-container w-full" onClick={ logout }>
                ...
                <nav className="flex justify-around items-center gap-2 fixed bottom-0 left-0 right-0 z-10 bg-neutral-100 border border-neutral-400 shadow-inner lg:static lg:bg-transparent lg:flex-col lg:items-start lg:border-none lg:shadow-none lg:mt-12">
                    ...
                    <div className="lg:w-full">
                        <NavLink to="/messages" className={({ isActive }) => isActive ? "nav-item-container nav-active" : "nav-item-container"}>
                            <svg className="nav-icon" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" aria-hidden="true">
                    ...
                    <div className="lg:w-full">
                        <NavLink to="/friends" className={({ isActive }) => isActive ? "nav-item-container nav-active" : "nav-item-container"}>
                            <svg className="nav-icon" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" aria-hidden="true">
                    ...
                    <div className="nav-item-container lg:hidden">
                        <button className="button" onClick={ toggleSecondaryNav }>
            ...
            {postModal.addPost.open && (
                <ModalWrapper fadeOut={ postModal.addPost.fadeOut } toggleModal={ () => handlePopups({ modalKey: "addPost", setModals: setPostModal }) }>
                    <AddPostModal toggleModal={ () => handlePopups({ modalKey: "addPost", setModals: setPostModal }) } />
                </ModalWrapper>
